...
// 首页带入的本地预览流（objectURL）无服务端地址，提交前统一上传换取可访问 URL
const resolveRemoteUrl = async (img: { id: number; url: string }) => {
  if (/^https?:\/\//.test(img.url)) return img;
  try {
    const blob = await fetch(img.url).then((res) => res.blob());
    const formData = new FormData();
    formData.append("file", blob, `reference-${Date.now()}-${img.id}.png`);
    const res = await API.uploadFile(formData);
    return { id: img.id, url: res.data.fileUrl };
  } catch (e) {
    console.error("本地图片上传失败:", e);
    return img;
  }
};
  ...
  (
    ...
    useEffect(() => {
      ...
      setReferenceImages((prev) => {
        ...
        return [
          ...prev,
          ...initialImages.filter((img) => !existing.has(img.url)),
        ];
      });
    }, [initialImages]);
    ...
    const handleSubmit = useCallback(async () => {
      ...
      try {
        ...
        for (const img of referenceImages) {
          // 首页带入的本地预览图无服务端地址，先上传换取可访问 URL
          const originImg = await resolveRemoteUrl(img);
          const boxes = boxesMap[img.url];
          if (!boxes || boxes.length === 0) {
            originImageList.push(originImg);
            continue;
          }
          try {
            const blob = await bakeBoxesToImage(originImg.url, boxes);
            if (!blob) {
              fallbackCount += 1;
              originImageList.push(originImg);
              continue;
